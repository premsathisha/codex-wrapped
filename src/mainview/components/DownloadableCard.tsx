...
export const buildDownloadableCardFileName = (title: string, date: Date, filenamePart?: string): string => {
	...
	return `codex-wrapped-${cardName}-${datePart}.png`;
};
...
const DownloadableCard = ({ title, filenamePart, children }: DownloadableCardProps) => {
	...
	const handleDownload = async () => {
		...
		const exportAnchors = Array.from(cardTarget.querySelectorAll<HTMLElement>('[data-export-scroll-anchor="end"]'));
		...
		try {
			...
			const fileName = buildDownloadableCardFileName(title, new Date(), filenamePart);
