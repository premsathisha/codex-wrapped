...
describe("normalizeSession costs", () => {
	...
	test("preserves lineage parent ids and token count fingerprints", () => {
		const result = normalizeSession({
			...makeRawSession("session-lineage", [
				makeEvent({
					id: "token-count-event",
					kind: "meta",
					tokenCountFingerprint: "fingerprint-1",
				}),
			]),
			metadata: {
				...makeRawSession("session-lineage", []).metadata,
				lineageParentId: "parent-thread-1",
			},
		});

		expect(result.session.lineageParentId).toBe("parent-thread-1");
		expect(result.events[0]?.tokenCountFingerprint).toBe("fingerprint-1");
	});
