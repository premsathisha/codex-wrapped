...
const getDashboardSummaryFromStore = async (dateFrom?: string, dateTo?: string): Promise<DashboardSummary> => {
	const daily = await readDailyStore();
...
const isAbsoluteCommandPath = (command: string): boolean => command.startsWith("/") || /^[a-zA-Z]:[\\/]/.test(command);
...
const exportFullSharePdf = async (url: string): Promise<{ path: string; browser: string }> => {
	...
	if (candidates.length === 0) {
		throw new Error("No supported browser found for automatic PDF export. Install Chrome, Chromium, Edge, or Brave.");
	}
...
export const startWebServer = async (options: StartWebServerOptions = {}): Promise<Bun.Server<unknown>> => {
	...
	if (!viteUrl && !existsSync(join(staticDir, "index.html"))) {
		throw new Error(`Missing ${join(staticDir, "index.html")}. Run "bun run build" before launching the web app.`);
	}
