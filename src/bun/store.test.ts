...
describe("rawAggregationMetaNeedsTimeZoneBackfill", () => {
	test("returns true when meta is missing or malformed", () => {
		...
		expect(
			rawAggregationMetaNeedsTimeZoneBackfill({ version: "1", timeZone: "America/Los_Angeles" }, "America/Los_Angeles"),
		).toBe(true);
	});

	test("returns true when stored aggregation meta version is stale", () => {
		expect(
			rawAggregationMetaNeedsTimeZoneBackfill({ version: 3, timeZone: "America/Los_Angeles" }, "America/Los_Angeles"),
		).toBe(true);
	});

	test("returns true when timezone changes", () => {
		expect(rawAggregationMetaNeedsTimeZoneBackfill({ version: 4, timeZone: "UTC" }, "America/Los_Angeles")).toBe(true);
	});

	test("returns false for matching version and timezone", () => {
		expect(
			rawAggregationMetaNeedsTimeZoneBackfill({ version: 4, timeZone: "America/Los_Angeles" }, "America/Los_Angeles"),
		).toBe(false);
	});
});
