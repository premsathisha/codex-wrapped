...
import { buildModelSummary, buildTopRepos, tokenTotalFromStats } from "./dashboardSummary";
...
describe("dashboardSummary", () => {
	...
	test("buildTopRepos includes tokens with stable ordering", () => {
	...
	test("buildModelSummary keeps high-token models inside the top 100 cutoff", () => {
		const byModel = new Map<string, DayStats>();
		...
		const summary = buildModelSummary(byModel);

		expect(summary).toHaveLength(100);
		expect(summary[0]?.model).toBe("big-one");
		expect(summary.some((row) => row.model === "big-one")).toBe(true);
		expect(summary.some((row) => row.model === "small-99")).toBe(false);
	});
});
