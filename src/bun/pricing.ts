...
const toModelsDevPricingDataset = (raw: unknown): ModelsDevPricingDataset => {
	...
	for (const [providerKey, providerData] of Object.entries(root)) {
		...
		const providerIdRaw =
			typeof providerRecord.id === "string" && providerRecord.id.trim().length > 0 ? providerRecord.id : providerKey;
		...
		for (const [modelKey, modelData] of Object.entries(modelsRecord)) {
			...
			const modelIdRaw =
				typeof modelRecord.id === "string" && modelRecord.id.trim().length > 0 ? modelRecord.id : modelKey;
