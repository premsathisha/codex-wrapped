...
interface SourcedCanonicalHistoryFact {
  backupId: string;
  fact: CanonicalHistoryFact;
}
...
const normalizeFact = (value: unknown): CanonicalHistoryFact | null => {
  ...
  if (
    !bucketStartUtc ||
    !dimensionKind ||
    !["all", "source", "model", "repo", "tool"].includes(dimensionKind)
  ) {
    return null;
  }

  const dimensionKey = normalizeDimensionKey(dimensionKind as HistoryDimensionKind, toNullableString(value.dimensionKey));
...
const normalizeImportHistoryStore = (value: unknown): ImportHistoryStore => {
  ...
  for (const [backupId, rawFacts] of Object.entries(value.factsByBackupId)) {
    ...
    factsByBackupId[backupId] = rawFacts.map(normalizeFact).filter((fact): fact is CanonicalHistoryFact => fact !== null);
  }
...
const compareBackupsForRecency = (
  left: ImportedBackupManifest,
  right: ImportedBackupManifest,
): number => {
...
const factIdentityKey = (fact: CanonicalHistoryFact): string =>
  `${fact.bucketStartUtc}|${fact.dimensionKind}|${fact.dimensionKey}`;
...
const selectEffectiveHistory = (
  ...
  const scanFactsByDate = groupFactsByLocalDate(
    scanFacts
      .filter((fact) => fact.dimensionKind === "all")
      .map((fact) => ({ fact })),
    timeZone,
  );
...
export const importBackupCsv = async (
  filename: string,
  csv: string,
  timeZone: string,
  ...
  if (duplicate) {
    return {
      recognized: true,
      duplicate: true,
      backup: toImportedBackupSummary(duplicate, beforeSelection.activeBackupIds, beforeSelection.contributingBackupIds),
  ...
  const simulatedSelection = selectEffectiveHistory(
    scanStore.facts,
    simulatedBackups,
    simulatedImportFactsByBackupId,
    {
      timeZone,
      overlapPolicy: "local-wins",
    },
