...
import { createEmptyDayStats, type DailyAggregateEntry, type DailyStore, type DayStats } from "./store";
...
const formatDateKey = (date: Date, formatter: Intl.DateTimeFormat): string => {
	...
	if (!year || !month || !day) {
		const fallbackMatch = /^\s*(\d{1,2})\/(\d{1,2})\/(\d{4})\s*$/.exec(formatter.format(date));
		if (fallbackMatch) {
			const [, fallbackMonth, fallbackDay, fallbackYear] = fallbackMatch;
			return `${fallbackYear}-${String(Number(fallbackMonth)).padStart(2, "0")}-${String(Number(fallbackDay)).padStart(2, "0")}`;
		}
...
const toEventDateKey = (event: SessionEvent, session: Session, dateFormatter: Intl.DateTimeFormat): string => {
...
const toEventHourKey = (event: SessionEvent, session: Session, hourFormatter: Intl.DateTimeFormat): string | null => {
	const parsed = parseTimestamp(resolveEventTimestamp(event, session));
	if (!parsed) return null;
	return formatHourKey(parsed, hourFormatter);
};

export const resolveAggregationTimeZone = (timeZone?: string): string => resolveFormatterTimeZone(timeZone);
...
export const aggregateSessionsByDate = (sessions: Session[], options: AggregateSessionsOptions = {}): DailyStore =>
	aggregateEntriesByDate(sessions, options);
