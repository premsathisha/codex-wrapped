...
const tokenizeRepo = (repo: string): Set<string> => {
	...
	return new Set(
		normalized
			...
			.filter((token) => token.length >= 4 && !/^\d+$/.test(token) && !REPO_STOP_TOKENS.has(token)),
	);
};
...
export const buildModelSummary = (byModelMap: Map<string, DayStats>): DashboardSummary["byModel"] =>
	[...byModelMap.entries()]
		.map(([model, stats]) => ({
			model,
		...
		.sort((left, right) => {
			const leftTokens = tokenTotalFromStats(byModelMap.get(left.model) as DayStats);
			const rightTokens = tokenTotalFromStats(byModelMap.get(right.model) as DayStats);
