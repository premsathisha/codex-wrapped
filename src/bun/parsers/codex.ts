...
const extractTokenCountFingerprint = (payload: Record<string, unknown> | null): string | null => {
	const info = asRecord(payload?.info);
	const totalTokenUsage = normalizeTokenUsage(info?.total_token_usage);
	const lastTokenUsage = normalizeTokenUsage(info?.last_token_usage);
	const totalCostUsd = toFiniteNumber(
		info?.total_cost_usd ??
			info?.totalCostUsd ??
			info?.totalCostUSD ??
	...
	if (!totalTokenUsage && !lastTokenUsage && totalCostUsd === null && lastCostUsd === null) {
		return null;
	}
...
const createEvent = (
	...
	options: {
		...
		tokens?: SessionEvent["tokens"];
		costUsd?: number | null;
		tokenCountFingerprint?: string | null;
	} = {},
): SessionEvent => {
	const payload = asRecord(record.payload);
	...
	return {
		...
		tokenCountFingerprint: options.tokenCountFingerprint ?? null,
	};
};
		...
		try {
			...
			for (let lineIndex = 0; lineIndex < records.length; lineIndex += 1) {
				...
				if (type === "session_meta") {
					...
					const forkedFromId = getString(payload?.forked_from_id);
				...
				if (type === "event_msg") {
					...
					if (messageType === "token_count") {
						const tokenCountFingerprint = extractTokenCountFingerprint(payload);
