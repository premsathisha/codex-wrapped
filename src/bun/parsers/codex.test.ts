...
describe("codexParser", () => {
	...
	test("counts token_count events once and ignores duplicate cumulative snapshots", async () => {
		...
		try {
			...
			const tokenCountEvents = parsed.events.filter((event) => event.tokenCountFingerprint !== null);
			expect(tokenCountEvents).toHaveLength(3);
			expect(tokenCountEvents[0]?.tokenCountFingerprint).toBe(tokenCountEvents[1]?.tokenCountFingerprint);
			expect(tokenCountEvents[1]?.tokenCountFingerprint).not.toBe(tokenCountEvents[2]?.tokenCountFingerprint);
	...
	test("marks spawned subagent sessions from session metadata", async () => {
		...
		try {
			...
			expect(parsed?.metadata.lineageParentId).toBe("parent-session");
			expect(normalizeSession(parsed as NonNullable<typeof parsed>).session.isSubagent).toBe(true);
			expect(normalizeSession(parsed as NonNullable<typeof parsed>).session.lineageParentId).toBe("parent-session");
		} finally {
			rmSync(fixtureDir, { recursive: true, force: true });
		}
	});
