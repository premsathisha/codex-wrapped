...
const formatFallbackDateFromEnUs = (value: string, date: Date): string => {
...
export const toISODateInTimeZone = (date: Date, timeZone?: string): string => {
	...
	if (!year || !month || !day) {
		return formatFallbackDateFromEnUs(formatter.format(date), date);
	}
...
export const calculateLongestStreakFromDates = (activeDates: Set<string>, dateFrom: string, dateTo: string): number => {
